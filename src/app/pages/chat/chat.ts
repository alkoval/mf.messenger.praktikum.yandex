...
export class ChatPageComponent extends BaseComponent implements OnInit {
  ...
  public onInit(): void {
    this.setProps({
      mdAddUser: new ModalAddUserComponent(
        {
          root: "",
          field: new FormField(
            "text",
      ...
      mdNewDialog: new ModalNewDialogComponent(
        {
          root: "",
          field: new FormField(
            "text",
      ...
      chatDialogsList: new ChatDialogListComponent(
        { root: "", dialogs: [] },
        this.templator
      ),
    ...
    this.profileService
      .subscribe()
      .on(PROFILE_EVENTS.PROFILE_UPDATE, this.updateProfile.bind(this));
  ...
  public prerenderChildrens(): void {
    if (this.getProps().mdAddUser) {
      const modals = [
        this.getProps().mdAddUser,
        this.getProps().mdDelUser,
        this.getProps().mdNewDialog,
      ];
      this.renderToRoot(modals);

      this.renderToSelector(
        [this.getProps().chatDialogsList],
        ".chat__sidebar"
      );

      this.getProps().chatHistory.setProps({
        root: null,
        mdAddUser: this.getProps().mdAddUser,
        mdDelUser: this.getProps().mdDelUser,
      });
  ...
  public subscribe(): void {
    const mdNewDialog = this.getElement().querySelectorAll(
      ".chat__toolbar .chat__link"
    )[0];
    if (mdNewDialog) {
      mdNewDialog.addEventListener("click", () => {
        this.showMdAddNewDialog();
      });
    }

    const profileLink = this.getElement().querySelectorAll(
      ".chat__toolbar .chat__link"
    )[1];
    if (profileLink) {
    ...
    const filter = this.getElement().querySelector(
      ".chat__toolbar .single-field__input"
    );
    if (filter) {
      filter.addEventListener("keyup", (e) => {
        this.filter(e!.target as HTMLInputElement);
      });
    }
  }
