...
export class ChatService {
  ...
  public loadDialogs(): Promise<unknown> {
    return this.chatAPI.request().then((response) => {
      ...
      for (const itemRes of JSON.parse(response as string)) {
  ...
  public addUserToDialog(id: number): void {
    ...
    if (!exist) {
      ...
      for (const profile of this.selectedDialog!.profiles) {
  ...
  public getProfilesForDelete(): Profile[] {
    ...
    if (
      ...
      profiles = this.selectedDialog.profiles.filter(
        (p) => p.id !== this.profileService.getProfile()!.id
      );
    }
  ...
  private setUsersDialog(id: number): Promise<unknown> {
    return this.chatUsersAPI.request(id).then((response) => {
      if (response) {
        const users: UserResponse[] = JSON.parse(response as string);
