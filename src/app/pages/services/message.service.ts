...
export class MessageService {
    ...
    public connect(archive: boolean): void {
        ...
        if (this.chatService.getSelectedDialog()) {
            ...
            this.chatTokenApi.request(dialogId).then(
                response => {
    ...
    public intercepter(data: DialogMessage | ArhiveMessage[]): void {
        if (!Array.isArray(data)) {
            if (data.type === 'message') {
                const profile = this.chatService.getSelectedDialog()?.profiles.find(e => e.id === data.userId);
            ...
            for (let item of data) {
                const profile = this.chatService.getSelectedDialog()?.profiles.find(e => e.id === item.user_id);
