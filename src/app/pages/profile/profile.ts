...
export class ProfilePageComponent extends BaseComponent implements OnInit {
  ...
  public subscribe(): void {
    ...
    const hideMd = this.getElement().querySelector(
      ".modal__button_bg_dark-green"
    );
    if (hideMd) {
      hideMd.addEventListener("click", () => {
        this.changeAvatar();
      });
    }

    const inputFile = this.getElement().querySelector(
      ".modal__file-upload__input"
    );
    if (inputFile) {
      inputFile.addEventListener("change", (e) => {
        this.checkFile(e.target as HTMLInputElement);
      });
    }
    ...
    const changeProfileLink = this.getElement().querySelectorAll(
      ".profile__footer .profile__group-link"
    )[0];
    if (changeProfileLink) {
      changeProfileLink.addEventListener("click", () => {
        this.router.go("/change-profile");
      });
    }

    const changePasswordLink = this.getElement().querySelectorAll(
      ".profile__footer .profile__group-link"
    )[1];
    ...
    const logoutLink = this.getElement().querySelectorAll(
      ".profile__footer .profile__group-link"
    )[2];
    if (logoutLink) {
  ...
  public changeAvatar(): void {
    ...
    if (inputFile) {
      if (this.checkFile(inputFile as HTMLInputElement)) {
        ...
        const avatar = this.getElement().querySelector(
          ".modal__file-upload__input"
        )! as HTMLInputElement;
  ...
  public checkFile(target: HTMLInputElement): boolean {
    ...
    const err = this.getElement().querySelector(
      ".modal__text, .modal__text_bg_red"
    );
