...
export abstract class BaseComponent implements Component {
  ...
  private registerEvents(): void {
    ...
    this.eventBus.on(
      EVENTS.FLOW_AFTER_RENDER,
      this.prerenderChildrens.bind(this)
    );
    ...
    this.eventBus.on(
      EVENTS.SUBSCRIBE_ON_CHILDRENS,
      this.subscribeOnChildrens.bind(this)
    );
  }
  ...
  public renderToSelectorTop(
    childrens: BaseComponent[],
    selector: string
  ): void {
    if (childrens.length > 0) {
      const root = this.elem.querySelector(selector);
