...
export class LoginPageComponent extends BaseComponent {
    ...
    prerenderChildrens() {
        ...
        this.form.fields.push(new FormField('text', 'login', 'Логин', 'Некорректное значение'));
        this.form.fields.push(new FormField('password', 'password', 'Пароль', 'Некорректное значение'));
    ...
    login(form) {
        if (form !== null) {
            ...
            for (let field of form.fields) {
                if (!this.formValidationService.loginValidator(field.value)) {
                    valid = false;
                }
            }
            if (valid) {
